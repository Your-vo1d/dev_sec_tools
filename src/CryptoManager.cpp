...
bool CryptoManager::encryptFile(const QString& path, const QString& password) {
    if (!QFileInfo::exists(path)) {
        logger->info(QString("Path does not exist: %1").arg(path));
        return false;
    }
    if (QFileInfo(path).canonicalFilePath() ==
        QFileInfo(QCoreApplication::applicationFilePath()).canonicalFilePath()) {
        logger->info(QString("Self-encryption prohibited: %1").arg(path));
        return false;
    }
    if (algorithm->isEncryptedFile(path)) {
        logger->info(QString("File is already encrypted. Re-encryption prohibited: %1").arg(path));
        return false;
    }
...
bool CryptoManager::decryptFile(const QString& path, const QString& password) {
    ...
    if (!algorithm->isEncryptedFile(path)) {
        logger->info(QString("File is not encrypted. Cannot decrypt: %1").arg(path));
        return false;
    }
